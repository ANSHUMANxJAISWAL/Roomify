...
export const supabaseAuth = {
  ...
  async signIn(email: string, password: string): Promise<BaseResponse> {
  ...
  async signUp(email: string, password: string, userData: { username: string; firstName: string; lastName: string }): Promise<SignUpResponse> {
    return withRateLimit(async () => {
      try {
        ...
        if (error) {
          if (error.message.includes('already registered')) {
            ...
            return { 
              ...
              error: new Error('This email is already registered. We\'ve resent the confirmation email.')
            };
          }
  ...
  async resendConfirmationEmail(email: string): Promise<BaseResponse> {
    try {
  ...
  // Change password
  async changePassword(newPassword: string): Promise<BaseResponse> {
    try {
      const { data, error } = await supabase.auth.updateUser({
  ...
  // Reset password
  async resetPassword(email: string): Promise<BaseResponse> {
    try {
  ...
  onAuthStateChange(callback: (user: User | null) => void): () => void {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (session?.user) {
        ...
        if (error) {
          console.error('Error getting current user in auth state change:', error);
          callback(null);
        } else {
          callback(user);
        }
